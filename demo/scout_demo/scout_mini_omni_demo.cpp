...
int main(int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        std::cout << "Usage: demo_scout_mini_omni_robot <interface>"
                  << std::endl
                  << "Example 1: ./demo_scout_mini_omni_robot can0"
                  << std::endl;
        return -1;
    }
    ...
    while (true) {
        ...
        std::cout << "control mode: "
                  ...
                  << " , error code: " << std::hex
                  << state.system_state.error_code << std::dec
        ...
        if (scout->GetParserProtocolVersion() == ProtocolVersion::AGX_V1) {
            for (int i = 0; i < 4; ++i) {
                printf(
                    "motor %d: current %f, rpm %d, driver temp %f, motor temp "
                    "%f\n",
